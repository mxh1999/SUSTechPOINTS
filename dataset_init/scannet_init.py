...
OBJ_CLASS_NAMES = ('wall', 'chair', 'floor', 'table', 'door', 'couch', 'cabinet', 'shelf', 'desk', 'office chair', 'bed', 'pillow', 'sink', 'picture', 'window', 'toilet', 'bookshelf', 'monitor', 'curtain', 'book', 'armchair', 'coffee table', 'box',
...
BLOCKED_CLASS_NAMES = ('wall', 'floor', 'ceiling')
...
def export_one_scan(scan_name,
    ...
    for i in range(aligned_bboxes.shape[0]):
        ...
        tmp = np.where(OBJ_CLASS_IDS == int(label))[0][0]
        cls_name = OBJ_CLASS_NAMES[tmp]
        if cls_name in BLOCKED_CLASS_NAMES:
            continue
